...
// --------------------------------------
void createSuspects(vector<Suspect*> &s); // CONSTRUCTS ALL FIVE SUSPECTS
void setAnswers(vector<Suspect*> &s); // LOADS ALL FIVE SUSPECTS' DIFFERENT POSSIBLE ANSWERS
void chooseGuiltyPerson(vector<Suspect*> &s); // RANDOMLY CHOOSES ONE OF THE FIVE SUSPECTS TO BE THE MURDERER
void introduction();
...
int main()
{
	vector<Suspect*> Suspects; // Create a vector to hold the suspects

	createSuspects(Suspects); // CreateSuspects() pushes each suspect back into Suspects vector
	setAnswers(Suspects); // Runs setAnswers function on Suspects vector

	// CHOOSE GUILTY SUSPECT
	srand(time(NULL));
	chooseGuiltyPerson(Suspects);

	introduction(); //... um.... SELF-EXPLANATORY

	int stillTalking = 1; // Assume their still talking to begin with...

	// Runs until player no longer wants to keep interviewing suspects
	while (stillTalking == 1)
	{
		Suspect* interviewee = whichSuspectTalkTo(Suspects); // "Plug in" which suspect they want to talk to
		
		// Interview() returns an integer (1 or 0)for whether they want to keep talking or not
		stillTalking = interview(interviewee);
	}

	guess(Suspects);

	// Delete all constructed suspect information from memory
...
// LOADS ALL FIVE SUSPECTS' DIFFERENT POSSIBLE ANSWERS
void setAnswers(vector<Suspect*> &s) // GIVES ALL FIVE SUSPECTS THEIR DIFFERENT POSSIBLE ANSWERS
{
	// SET BOB'S ANSWERS
...
// RANDOMLY CHOOSES ONE OF THE FIVE SUSPECTS TO BE THE MURDERER
...
// LETS PLAYER CHOOSE WHO TO INTERVIEW
...
// INTERVIEWS CHOSEN SUSPECT AND THEN RETURNS 1 OR 0 FOR CONTINUING INTERVIEWING
...
// LETS PLAYER GUESS WHO THE KILLER IS... FUN PART!!!
void guess(vector<Suspect*> &s)
{
	...
	if (s[choice - 1]->getGuiltyFlag() == false)
		cout << "So sorry detective... " << s[choice - 1]->getName() << " was actually innnocent," << 
		" and you sent them to jail." << endl << "YOU'RE FIRED!" << endl;
}